...
interface ListFunnelsInput {
  limit?: number;
}

class ListfunnelsTool extends MCPTool<ListFunnelsInput> {
  name = "listFunnels";
  ...
  async execute(input: ListFunnelsInput) {
    try {
      ...
      const response = await axios.get(
        `${apiBase}/workspaces/${workspaceId}/funnels`,
        {
          headers: {
      ...
      return {
        error: true,
        message: `Error listing funnels: ${(error as Error).message}`,
      };
    }
  }
}
